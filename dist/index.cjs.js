...
var Sister__default = /*#__PURE__*/_interopDefaultLegacy(Sister);
...
const Contents = (config) => {
  ...
  const eventEmitter = Sister__default['default']();
...
Contents.config = (userConfig = {}) => {
  ...
  const difference = ___default['default'].difference(Object.keys(userConfig), properties);
  ...
  const instanceConfig = ___default['default'].assign({}, defaultConfig, userConfig);
...
Contents.articles = (elements, articleName = Contents.articleName, articleId = Contents.articleId) => {
  return ___default['default'].map(elements, (element) => {
    const article = {};
...
Contents.tree = (articles, makeUniqueIDs = true, uniqueIDpool = []) => {
  ...
  ___default['default'].forEach(articles, (article) => {
    if (makeUniqueIDs) {
...
Contents.list = (tree, link) => {
  ...
  ___default['default'].forEach(tree, (article) => {
